...
export const useSales = () => {
    ...
    const updateSaleMutation = useMutation({ mutationFn: updateSaleDB, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['sales'] }); toast({ title: "Éxito", description: "Venta actualizada correctamente." }); }, onError: (error) => { toast({ title: "Error", description: error.message, variant: "destructive" }) }, });
    ...
    const chartDataByDate = useMemo(() => {
        ...
        sales.forEach((sale) => {
            ...
            if (!salesMap[date]) { salesMap[date] = { revenue: 0, commission: 0 }; }
            salesMap[date].revenue += sale.totalAmount;
            salesMap[date].commission += (sale.commissionAmount || 0);
        });
        return Object.entries(salesMap).map(([date, data]) => ({ date, revenue: data.revenue, commission: data.commission })).slice(-7);
    }, [sales]);

    return { sales, isLoading: isLoadingSales, getTotalCommissions: () => totalCommissions, getChartDataByDate: () => chartDataByDate, updateSale: updateSaleMutation.mutate, clearAll: clearAllMutation.mutate };
};
